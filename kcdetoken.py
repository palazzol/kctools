...
parser.add_argument('-c','--program-string-offset', type=int, default=0, help='BASIC variables are stored to tape separately from BASIC programs.  A program can be updated and move the location of the strings inside of the code without the independently stored variables being updated, causing wrong addresses to be used for program strings.  Use this option to set a positive or negative number to shift the address of program strings during detokenizing.  If the CFG file or if kcmapmemory.py output a messages similar to "Shifting start address of variables from $1773 to existing $1f69" then the different is a starting point for what -c should be set to.  This option does not affect other types of strings.')
